...
void display()
{
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, 16, 16, 0, GL_RGB, GL_UNSIGNED_BYTE, secondaryFrameBuffer);
	// Bind the texture
    glBindTexture(GL_TEXTURE_2D, textureID);
...
{
	if(key=='e'){exportPalette("paletteArt.bin",palettes);}
	if (key=='o'){openFileDialogAndLoadFile();if (fileSize <= MAX_FILE_SIZE) initialize();}
