...
;unsigned short combinedPaletteData;
...
unsigned char frameBufferPLN0[1000000]; //0x010280
...
combinedPaletteData = (getRED) + (getGREEN << 5) + (getBLUE << 10);
frameBufferPLN0[imageWriteOffset] = combinedPaletteData;
frameBuffer[imageWriteOffset] = (combinedPaletteData >> 8);
imageWriteOffset ++;
i += 3;
}
}
...
void display()
{
	...
	{
	combinedPaletteData = (frameBufferPLN0[dataOffset + getLineOffset]) + ((frameBuffer[dataOffset + getLineOffset]) << 8);
    secondaryFrameBuffer[frameBufferOffset] = (combinedPaletteData & 31) << 3;
    secondaryFrameBuffer[frameBufferOffset + 1] = ((combinedPaletteData >> 5) & 31) << 3;
    secondaryFrameBuffer[frameBufferOffset + 2] = ((combinedPaletteData >> 10) & 31) << 3;
    dataOffset ++;
	}
	else
	{
...
{
	if(key=='e'){exportImage("15bppImagePLN0.bin",frameBufferPLN0,(fileSize - headerOffset) / 3); exportImage("15bppImagePLN1.bin",frameBuffer,(fileSize - headerOffset) / 3);}
